...
test('Login Correto', async ({ page }) => {
    ...
    await page.fill('#_USUSEN', process.env.SENHA!);

    //Clica no botão de Login
    await page.locator('//*[@id="TABLE"]/tbody/tr[6]/td[3]/input[1]').click();
        

    const confirmar = page.locator('//*[@id="TABLE_DERRUBA_SESSAO"]/tbody/tr[5]/td[3]/input[1]');

    if (confirmar) {
        await confirmar.click();
    }

    await expect(page).toHaveURL('https://www.moozcobranca.com.br/homologacao/servlet/hbranco');
